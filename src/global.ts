...
function getBufferUsage(usage: string) {
...
async function fetchBin(url: string): Promise<Float32Array> {
  ...
  function sampleFromDistribution(probs: Float32Array | number[]): number {
  ...
  function cpuSoftmax(logits: Float32Array, temperature: number = 1.0): Float32Array {
  ...
  const zeros = (dim: number): Float32Array => new Float32Array(dim).fill(0);
  ...
  function leastPrimeFactor(n: number, start: number = 2): number {
